...
export type PhoneNumberString<A, T extends string = ''> = A extends `${
  | 2
  | 3
  | 4
  | 5
  ...
  | 9}${infer B}`
  ? B extends string
    ? LengthOfString<B> extends 9
      ? `${NaturalNumber}${DigitString<B>}`
      : never
    : never
  : T;
...
export type OpenDay = { active: boolean; hours: OpenHours };
...
export type CustomerInfo = {
  ...
  // preferredFrequencyType: StatusCode2;
  // referralSourceId: ID | null;
  // preferredDay: Array<WeekDayCode>;
  // preferredTime: TimeFrame;
  // // No, Yes
  // autoEmail: StatusCode2;
  // // No, Yes
  // autoMessage: StatusCode2;
};
...
export type AppointmentData = {
  ...
  repeat: number;
  // Pending-Accept, Accepted ,Cancelled
  status: StatusCode3;
...
export type Service = {
  ...
  price: number;
  active: boolean;
  image: null | string | { [key: string]: string };
  deposit?: number;
  description: string;
...
export type EmbeddedButton = { onClick: MouseEventHandler; icon: string };
...
export type FormInputElement = HTMLInputElement & { files: File[] };
